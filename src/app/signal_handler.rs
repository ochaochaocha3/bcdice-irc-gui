...
pub fn connect_hostname_entry_changed(
    ...
    w.hostname_entry
        .connect_changed(glib::clone!(@strong irc_bot_config => move |e| {
            let mut c = irc_bot_config.borrow_mut();
...
pub fn connect_port_spin_button_value_changed(
    ...
    let w = widgets.borrow();
    w.port_spin_button
        .connect_value_changed(glib::clone!(@strong irc_bot_config => move |b| {
            let mut c = irc_bot_config.borrow_mut();
...
pub fn connect_connect_disconnect_button_clicked(
    ...
    let w = widgets.borrow();
    w.connect_disconnect_button
        .connect_clicked(glib::clone!(@strong irc_bot_config => move |_| {
            println!("irc_bot_config: {:?}", irc_bot_config.borrow());
        }))
}
